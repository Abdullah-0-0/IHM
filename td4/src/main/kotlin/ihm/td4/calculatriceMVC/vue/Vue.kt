...
class Vue: GridPane() {
    ...
    fun contr_chfi(controle : EventHandler<ActionEvent>){
        for (i in 0 until tabButton.size-1){
            this.tabButton[i].onAction = controle
            println("${this.tabButton[i].text}je passe dans l'évent au moins  ")
        }
    }
    fun contr_effacer(controle: EventHandler<ActionEvent>){
        this.tabButton[15].onAction = controle
    }

    fun contr_res(controle: EventHandler<ActionEvent>){
        this.tabButton[14].onAction = controle
    }
